...
/**
 * @description: 范围算法 - Database精确分片策略
 *
 * @author: zhang xuan
 * @time: 2020/4/16
 */
@Slf4j
public final class RangePreciseShardingDatabaseAlgorithm implements PreciseShardingAlgorithm<Long> {
    ...
    public String doSharding(final Collection<String> databaseNames, final PreciseShardingValue<Long> shardingValue) {
        ...
        for (Pair<String,Integer> each : CommonUtils.nameToSuffixPair(databaseNames)) {
            if (each.getValue()==shardingValue.getValue() / rangeDsRecordsSize) {
                return each.getKey();
            }
        }
