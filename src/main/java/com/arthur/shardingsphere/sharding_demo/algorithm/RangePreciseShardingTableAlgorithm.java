...
/**
 * @description: 范围算法 - Table精确分片策略
 *
 * @author: zhang xuan
 * @time: 2020/4/16
 */
@Slf4j
public final class RangePreciseShardingTableAlgorithm implements PreciseShardingAlgorithm<Long> {
    ...
    public String doSharding(final Collection<String> tableNames, final PreciseShardingValue<Long> shardingValue) {
        ...
        for (Pair<String,Integer> each : CommonUtils.nameToSuffixPair(tableNames)) {
            if (each.getValue()==shardingValue.getValue() % rangeDsRecordsSize / GlobalVar.RANGE_TABLE_RECORDS_SIZE) {
                return each.getKey();
            }
        }
        log.info("范围精确分片策略：没找到与分片键匹配的库名! {} : {} = {}",shardingValue.getLogicTableName(),shardingValue.getColumnName(),shardingValue.getValue());
