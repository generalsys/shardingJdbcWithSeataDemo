...
@Slf4j
public final class RangeRangeShardingDatabaseAlgorithm implements RangeShardingAlgorithm<Long> {
    ...
    public Collection<String> doSharding(final Collection<String> databaseNames, final RangeShardingValue<Long> shardingValueRange) {
        ...
        if (shardingValueRange.getValueRange().hasLowerBound()) {
            if (shardingValueRange.getValueRange().lowerBoundType().equals(BoundType.CLOSED)) {
                lowerBound = shardingValueRange.getValueRange().lowerEndpoint() / rangeDsRecordsSize;
            }else {
                lowerBound = (shardingValueRange.getValueRange().lowerEndpoint() + 1) / rangeDsRecordsSize;
            }
        }
        if (shardingValueRange.getValueRange().hasUpperBound()) {
            if (shardingValueRange.getValueRange().upperBoundType().equals(BoundType.CLOSED)) {
                upperBound = shardingValueRange.getValueRange().upperEndpoint() / rangeDsRecordsSize;
            }else {
                upperBound = (shardingValueRange.getValueRange().upperEndpoint() - 1) / rangeDsRecordsSize;
            }
        }
        //匹配的库返回结果
